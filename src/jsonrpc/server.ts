...
import { SimpleJsonRpcConnection } from "./connection";
...
export function runJsonRpcServer(
  ...
  wss.on("connection", (ws) => {
    ...
    const context = {};
    const connection = new SimpleJsonRpcConnection(ws);
    ws.on("message", async function message(data) {
      ...
      let result: unknown = await handleRequest(jsonRpc, parsed, { connection, context });
      if (result instanceof Response) {
